...
-- Reach (optimized - only fires on closest ball, caches limbs, throttled)
...
local reachLastFire = 0
Connections.reach = RunService.Heartbeat:Connect(function()
    if not Config.reachOn then return end
    local now = tick()
    if now - reachLastFire < 0.05 then return end -- 20 times per sec max
    reachLastFire = now
    
    local ball = getClosestBall() -- only closest ball, not all
